...
async function startResearch(contract, summoner, nonce) {
    try {
    ...
    if (!config.dry_run) {
        await contract.methods.startResearch(summoner, config.days)
            .send({
                from: config.myAddress,
                gasPrice: gas_price,
                gas: "179043",
...
export default class Forest {
    async run() {
        ...
        for (const summoner of summoners) {
            ...
            if (level >= 2) {
                console.log(`Sending summoner ${summoner} level ${level} in the forests (V1 + V2)...`);
                await startResearch(forestV1, summoner, nonce++);
                await startResearch(forestV2, summoner, nonce++);
            }
        }
    };
